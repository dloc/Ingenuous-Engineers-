...
function user_has_reviews() {
    $user_reviews = get_user_reviews($_SESSION['userID']);
//    if(!empty($user_reviews)) {
//        $_SESSION['user_reviews'] = $user_reviews;
//        return true;
//    }
//    else return false;
    return !empty($user_reviews) ? true : false;
}

function get_user_reviews() {
    $stmt = $GLOBALS['gjdb']->prepare("SELECT * FROM gj_reviews WHERE userID = :userID");
