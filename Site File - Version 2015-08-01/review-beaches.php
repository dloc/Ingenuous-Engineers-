...
<div style="max-width: 90%; margin: 0 auto; padding-top: 5%;">
    ...
    <div align="center">
        ...
        <form method="POST" action="accept-review.php">
            <p><font color="white">Select Beach: </font>
...
<div id="container">
    ...
    <div style="padding-left: 3%; padding-right: 3%">
        ...
        <?php for ($i=($size-1); $i>=0; $i--) { ?>            
            <?php if($review_info[$i]["attraction"] === "Carlin Park") { ?>"<?php echo $review_info[$i]["review"];?>" -- <?php echo $review_info[$i]["firstName"]; ?><span> (<?php echo $review_info[$i]["stars"]; ?> stars)</span><br>
        <?php }
        }?>
        </p>
    </div>
    ...
    <div style="padding-left: 3%; padding-right: 3%">
        <p style="font-size: 0.75em">
        <?php for ($i=($size-1); $i>=0; $i--) { ?>            
            <?php if($review_info[$i]["attraction"] === "Coral Cove") { ?>"<?php echo $review_info[$i]["review"];?>" -- <?php echo $review_info[$i]["firstName"]; ?><span> (<?php echo $review_info[$i]["stars"]; ?> stars)</span><br>
        <?php }
        }?>
        </p>
    </div>
    ...
    <!-- Jupiter Beach Reviews -->
    <p>Jupiter Beach: <span>Average 
    <?php for ($i=0; $i<$size; $i++) { ?>            
    ...
    <div style="padding-left: 3%; padding-right: 3%">
        <p style="font-size: 0.75em">
        <?php for ($i=($size-1); $i>=0; $i--) { ?>            
            <?php if($review_info[$i]["attraction"] === "Jupiter Beach") { ?>"<?php echo $review_info[$i]["review"];?>" -- <?php echo $review_info[$i]["firstName"]; ?><span> (<?php echo $review_info[$i]["stars"]; ?> stars)</span><br>
        <?php }
        }?>
        </p>
    </div>
