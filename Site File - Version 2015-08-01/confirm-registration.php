...
if (strlen($pass1) < 4 || strlen($pass2) < 4) { ?>
    <div style='text-align:center; font-size:14pt; padding-top: 5%;'>Your password must be at least 4 characters long.<br>
        Please go back and re-enter.<br>
        <a href='signup.php' style='font-size:12pt' onClick='history.back();return false;'>GO BACK</a>
    </div>
<?php }
...
if (!ctype_alpha($firstName) || !ctype_alpha($lastName) || $pass1 != $pass2) { ?>
    <div style='text-align:center; padding-top: 5%;'>Your user account could not be created for the following reason(s):<br><br>
	    ...
	    <div style='text-align:center; padding-top: 5%;'><em><b>First/Last name can only contain letters.</b></em></div>
        <?php }
        if($pass1 != $pass2) { ?>
...
if (email_exists($email)) { ?>
    ...
    <div style='text-align:center;'><a href="login.php">Log In</a></div>
<?php }
...
if(!email_exists($email) && ctype_alpha($firstName) && ctype_alpha($lastName) && $pass1 == $pass2 && strlen($pass1) >= 4 && strlen($pass2) >=4 )	{
    $password = htmlspecialchars(md5($pass1));
	$register_data = array(
        'email'     => $email,
		'password' 	=> $password,     // Password is encrypted with MD5
        'firstName' => $firstName,
        'lastName'  => $lastName
	);
...
<div style='text-align:center; padding-top: 5%;'>Please <a href="login.php">log in</a> to <a href="booking.php">make travel arrangements</a> and <a href="reviews.php">read or write reviews</a></div>
